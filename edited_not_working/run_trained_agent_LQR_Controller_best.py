...
# === Utility functions ===
# Assuming the structure is: quadrotor (8) + pendulum (3) + goal (2)
# def parse_obs(obs):
    # pos = obs[0:3]
    # vel = obs[3:6]
    # orient = obs[6:8]  # s_theta, c_theta
    # ang = np.array([obs[8], 0.0, obs[9]])  # theta_dot, dummy, phi_dot
    # return pos, vel, orient, ang
...
def anomaly_detector(obs, prev_obs):
    ...
    if np.any(np.isnan(obs)) or np.any(np.isinf(obs)):
        print("[Anomaly Debug] NaN or Inf detected in observation.")
        print(f'Obs: {obs}')
        return "severe"

    if np.any(np.abs(orient[:2]) > MAX_TILT):
        #print(f"[Anomaly Debug] Tilt too large: s_theta={orient[0]:.3f}, c_theta={orient[1]:.3f}, "f"threshold={np.degrees(MAX_TILT):.2f}°")
        return "severe"

    vel_norm = np.linalg.norm(vel)
    ang_norm = np.linalg.norm(ang)
    if vel_norm > MAX_VEL or ang_norm > MAX_ANG_VEL:
        #print(f"[Anomaly Debug] Velocity anomaly: lin_vel={vel_norm:.2f}, ang_vel={ang_norm:.2f} "f"(thresholds: {MAX_VEL}, {np.degrees(MAX_ANG_VEL):.2f}°/s)")
        return "mild"

    vel_change = np.linalg.norm(vel - prev_vel)
    ang_change = np.linalg.norm(ang - prev_ang)
    if vel_change > DISTURB_THRESH or ang_change > DISTURB_THRESH:
        print(f"[Anomaly Debug] Sudden change detected: Δvel={vel_change:.2f}, Δang={ang_change:.2f} "
              f"(threshold: {DISTURB_THRESH})")
        return "mild"
...
def obs_to_state(obs):
    ...
    ph_dot = ang[2]
    return np.array([x, z, vx, vz, s_th, c_th, th_dot, s_ph, ph_dot])
...
def test_agent_combined(config, render_mode, model_path,
                        ...
                        manual_goal_position=None, live_plot=False, save_summary_plot=True):
    pygame.init()
    ...
    env = QuadPole2DWrapper(config, render_mode, manual_goal_position, final_eval=True)
    ...
    if mode == "intervene":
        ...
        try:
            ...
            try:
                ...
                print(f"LQR failed; safety policy disabled: {e}\n")
                K = None
                #raise ValueError
    ...
    for ep in range(n_episodes):
        ...
        pendulum_upright_once = False
        upright_trigger_step = None
        while not done:
            ...
            cooldown_passed = pendulum_upright_once and (step - upright_trigger_step >= 5)
            #if not in_safety and anomaly and cooldown_passed:
            if mode == "intervene" and not in_safety and anomaly:# and cooldown_passed:
                in_safety = True
                lqr_start_step = step
                print(f"[Step {step}] LQR takeover triggered by {anomaly} after cooldown.")
                
                
            #elif anomaly and not cooldown_passed:
                #print(f"[Step {step}] Anomaly ({anomaly}) blocked: cooldown in progress.")

            #if not in_safety and anomaly and pendulum_upright_once
                #in_safety = True
                #lqr_start_step = step
                #print(f"[Episode {ep+1}, Step {step}] → LQR takeover due to {anomaly}")
            #elif anomaly and not pendulum_upright_once:
                #print(f"[Episode {ep+1}, Step {step}] → Anomaly detected ({anomaly}) but LQR blocked until pendulum is upright at least once.")

            if mode == "intervene" and K is not None and in_safety:
                action = safety_policy(obs, K, x0)
                print('\n%%%%%%% Action taken by safety policy')
            else:
            ...
            if obs[8] < -0.92 and abs(obs[9]) < 0.2 and not pendulum_upright_once:
                pendulum_upright_once = True
                upright_trigger_step = step
                print(f"[Step {step}] Pendulum upright once triggered. Cooldown begins.")
        ...
        #goal_reach_time = env.env.goal_reached_time if env.env.goal_reached_time is not None else -1
