...
class QuadPole2D():
    def __init__(
            ...
            env_name = 'QuadPole2D',
            max_steps = 1000,
            timestep = 0.02, final_eval=False):
    ...
    def reset(self):
        ...
        s_phi, c_phi = self.state_dict['pendulum'][:2]
        reconstructed_phi = np.arctan2(s_phi, c_phi)
        print(f"[Reset] phi_init = {phi_init:.2f}, pendulum = {self.state_dict['pendulum']}")
        print(f"phi_init = {phi_init:.3f}, sin(phi_init) = {np.sin(phi_init):.3f}, cos(phi_init) = {np.cos(phi_init):.3f}")
    ...
    def _dynamics(self, state, control):
        ...
        new_state = [x_new, z_new, vx_new, vz_new, s_theta_new, c_theta_new, theta_dot_new, s_phi_new, c_phi_new, phi_dot_new]
